...
fly:
	...
	sw		$t8, PLANETS_REQUEST		# write planets array to $t8
	mul 	$a0, $a0, 24		
	add		$a0, $a0, $t8
	...
	move 	$a0, $t0					# a0 = diff x
	move 	$a1, $t1					# a1 = diff y
	jal 	sb_arctan					# arctan(y/x)
	lw 		$a0, 4($sp)
	sw 		$v0, ANGLE					# set angle
	li 		$t0, 1						
	sw 		$t0, ANGLE_CONTROL			# set angle control
	sub 	$s6, $s6, 1
	bge 	$s6, $zero, fly
	sw 		$t0, LANDING_REQUEST		# try to land
	lw 		$t0, LANDING_REQUEST		# get what planet bot is on 
	#sne 	$t1, $s5, $t0 				
	add 	$t0, $t0, 1
	sne 	$t0, $t0, $zero
	#and 	$t0, $t0, $t1
